...
static void udp_input(const uint8_t* data,
                      size_t len,
                      ip_addr_t src,
                      ip_addr_t dst,
  ...
  hexdump(stderr, entry->data, entry->len);
  if (!queue_push(&pcb->queue, entry)) {
    mutex_unlock(&mutex);
    errorf("quque_push failure");
    return;
  }
...
int udp_bind(int id, struct ip_endpoint* local) {
  ...
  if (exist) {
    errorf("pcb already exist local=%s",
...
ssize_t udp_sendto(int id,
  ...
  if (local.addr == IP_ADDR_ANY) {
    ...
    if (!iface) {
      errorf("iface not fount that can reach foreign address, addr=%s",
  ...
  if (!pcb->local.port) {
    for (p = UDP_SOURCE_PORT_MIN; p <= UDP_SOURCE_PORT_MAX; p++) {
      if (!udp_pcb_select(local.addr, hton16(p))) {
        ...
        debugf("dynamic assign local port, oport=%d", p);
        break;
      }
    }
    if (!pcb->local.port) {
      debugf("failed to dinamic assign local port");
      return -1;
    }
  }
...
ssize_t udp_recvfrom(int id,
  ...
  memory_free(entry);
  debugf("udp received len: %zu", len);
  return len;
}
