...
#define ETHER_TAP_IRQ INTR_BASE + 2
...
static int ether_tap_addr(struct net_device* dev) {
  ...
  if (ioctl(soc, SIOCGIFHWADDR, &ifr) == -1) {
    errorf("ifctl [SIOCGIFHWADDR]: %s, dev=%s", strerror(errno), dev->name);
    close(soc);
    return -1;
  }
...
static ssize_t ether_tap_write(struct net_device* dev,
                               const uint8_t* frame,
                               size_t flen) {
...
static ssize_t ether_tap_read(struct net_device* dev,
                              uint8_t* buf,
                              size_t size) {
  ssize_t len;
...
struct net_device* ether_tap_init(const char* name, const char* addr) {
  ...
  if (addr) {
    if (ether_addr_pton(addr, dev->addr) == -1) {
      errorf("ether_adddr_pton failure; invalid address, adddr=%s", addr);
      return NULL;
    }
  }
  ...
  if (net_device_register(dev) == -1) {
    errorf("net_device_register() in tap failure");
    memory_free(tap);
    return NULL;
  }
  intr_request_irq(tap->irq, ether_tap_isr, INTR_IRQ_SHARED, dev->name, dev);
  infof("ethernet device initialized(tap), dev=%s", dev->name);
  return dev;
}
