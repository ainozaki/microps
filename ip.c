...
ssize_t ip_output(uint8_t protocol,
  ...
  if (src == IP_ADDR_ANY) {
    errorf("ip routing not supported");
    return -1;
  } else {
    // find correspond iface for src
    iface = ip_iface_select(src);
    if (!iface) {
      errorf("cannot find iface for %s", ip_addr_ntop(src, addr, sizeof(addr)));
      return -1;
    }
    ...
    if ((dst & iface->netmask) != (iface->unicast & iface->netmask)) {
      if (dst != IP_ADDR_BROADCAST) {
        errorf("unreachable destination");
        return -1;
      }
    }
